...
`n_outlier`, which we will report. Note that when `maximum_normed_residual` is
inside a `summarize` function, we have to use the syntax `x = ...` to specify
the data on which to perform the maximum normed residual test.
