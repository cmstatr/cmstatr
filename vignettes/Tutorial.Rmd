...
Before we can acutally use the package, we'll need to load it. We'll also load
...
function can be used to normlize strength or modulus data to a certain
cured ply thickness. This function takes three arguments: the value to
normalize (ie. strenght or modulus), the measured thickness and the
...
signficane of the Anderson-Darling test is greater than $0.05$), we can
proceed to calculate a basis value based based on the assumption of normally
distributed data. The `cmstatr` package provides teh function `basis_normal`
...
After checking that there are a sufficinet number of conditions, batches and
...
normed resiudal test can be used for this. The `cmstatr` package provides the
