...
Function                        | Tests
--------------------------------|---------------------------
`ad_ksample()`                  | [Section 3.1](#cf-ad), [Section X.X.X](#c11-adk), [Section X.X](#cl-adk)
`anderson_darling_normal()`     | [Section X.X.X](#c11-ad), [Section X.X](#ods-adn)
`anderson_darling_lognormal()`  | [Section X.X.X](#c11-ad), [Section X.X](#ods-adl)
`anderson_darling_weibull()`    | [Section X.X.X](#c11-ad), [Section X.X](#ods-adw)
`basis_normal()`                | [Section X.X](#ods-nb)
`basis_lognormal()`             | [Section X.X](#ods-lb)
`basis_weibull()`               | [Section X.X](#ods-wb)
`basis_pooled_cv()`             | [Section X.X.X](#c12-pcv), [Section X.X.X](#c12-pcvmcv),
`basis_pooled_sd()`             | [Section X.X.X](#c12-psd), [Section X.X.X](#c12-psdmcv)
`basis_hk_ext()`                | [Section X.X.X](#c11-hk-wf), [Section X.X](#ods-hkb), [Section X.X](#ods-hkb2)
`basis_nonpara_large_sample()`  | [Section X.X](#ods-lsnb)
`basis_anova()`                 | [Section X.X.X](#c11-anova)
`calc_cv_star()`                |
`cv()`                          |
`equiv_change_mean()`           | [Section X.X](#ods-ecm)
`equiv_mean_extremum()`         | [Section X.X](#ods-eme)
`hk_ext_z()`                    | [Section X.X](#pf-hk), [Section X.X](#pf-hk2)
`hk_ext_z_j_opt()`              | [Section X.X](#pf-hk-opt)
`k_equiv()`                     | [Section X.X](#pf-equiv)
`k_factor_normal()`             | [Section X.X](#pf-kb), [Section X.X](#pf-ka)
`levene_test()`                 | [Section X.X.X](#c11-lbc), [Section X.X.X](#c11-lbb)
`maximum_normed_residual()`     | [Section X.X.X](#c11-mnr)
`nonpara_binomial_rank()`       | [Section X.X](#pf-npbinom), [Section X.X](#pf-npbinom2)
...
### X.X.X Maximum Normed Residual Test {#c11-mnr}
...
### X.X.X Anderson--Darling k--Sample Test {#c11-adk}
...
### X.X.X Anderson--Darling Tests for Distribution {#c11-ad}
...
### X.X.X Levene's Test (Between Conditions) {#c11-lbc}
...
expect_equal(res$f, 0.058, tolerance = 1e-3)
res
```

### X.X.X Levene's Test (Between Batches) #{c11-lbb}
...
### X.X.X Nonparametric Basis Values {#c11-hk-wf}
...
```{r}
hk_ext_z_j_opt(22, 0.90, 0.95)
```


### X.X.X Single-Point ANOVA Basis Value {#c11-anova}
...
### X.X.X Pooled SD A- and B-Basis {#c12-psd}
...
### X.X.X Pooled SD A- and B-Basis (Mod CV) {#c12-psdmcv}
...
### X.X.X Pooled CV A- and B-Basis {#c12-pcv}
...
### X.X.X Pooled CV A- and B-Basis (Mod CV) {#c12-pcvmcv}
This data set was input into CMH17-STATS and the Pooled CV method was selected
...
## X.X Anderson--Darling Test (Normal) {#ods-adn}
...
## X.X Anderson--Darling Test (Lognormal) {#ods-adl}
The following data set was randomly generated. When this is
entered into STAT17 [@STAT-17], that software gives the value
...
## Anderson--Darling Test (Weibull) {#ods-adw}
The following data set was randomly generated. When this is
entered into STAT17 [@STAT-17], that software gives the value
...
## X.X Normal A- and B-Basis {#ods-nb}
...
## X.X Lognormal A- and B-Basis {#ods-lb}
...
## X.X Weibull A- and B-Basis {#ods-wb}
...
## X.X Extended Hanson--Koopman A- and B-Basis {#ods-hkb}
...
## X.X Extended Hanson--Koopman B-Basis {#ods-hkb2}
...
## X.X Large Sample Nonparametric B-Basis {#ods-lsnb}
...
## X.X Acceptance Limits Based on Mean and Extremum {#ods-eme}
...
## Acceptance Based on Change in Mean {#ods-ecm}
...
## X.X Anderson--Darling K--Sample Test {#cl-adk}
...
# X. Comparison with Published Factors
...
## X.X Nonparametric B-Basis Extended Hanson--Koopman {#pf-hk}
...
## X.X Nonparametric A-Basis Extended Hanson--Koopman {#pf-hk2}
...
## X.X Factors for Small Sample Nonparametric B-Basis {#pf-hk-opt}
...
the factor ($k$) are different. Since both of these value are different,
the resulting B-Basis value will actually be similar.
...
## X.X Nonparametric B-Basis Binomial Rank {#pf-npbinom}
...
## Nonparametric A-Basis Binomial Rank {#pf-npbinom2}
...
## X.X Factors for Equivalency {#pf-equiv}
...
read.csv(system.file("extdata", "k1.vangel.csv",
  ...
  filter(n >= 5 & 0.005 <= alpha & alpha <= 0.1) %>% 
  rowwise() %>%
  mutate(k1_calc = k_equiv(alpha, n)[1]) %>%
  mutate(check = expect_equal(k1, k1_calc, tolerance = 5e-4)) %>%
  select(-c(check))
```
...
read.csv(system.file("extdata", "k2.vangel.csv",
  ...
  filter(n >= 5 & 0.005 <= alpha & alpha <= 0.1) %>% 
  rowwise() %>%
  mutate(k2_calc = k_equiv(alpha, n)[2]) %>%
  mutate(check = expect_equal(k2, k2_calc, tolerance = 5e-4)) %>%
  select(-c(check))
```
