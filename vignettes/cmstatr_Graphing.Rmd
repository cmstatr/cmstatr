...
can be loaded either on its own, or through the `tidyverse` meta-package, whic
...
the `ordered` function. For brevity, only the first few rows of the dataset
are displayed below.
...
method. This dataset happens to fail some of the diagnostic tests, but for the
...
distribution with the mean and varaince from the data is also plotted
...
the normal score. Normal plots are usful to investigate distributions of
the data.
