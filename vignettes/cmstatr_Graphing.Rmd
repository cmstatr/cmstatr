...
If both quantiles come from the same distribution, the points should
fall on a straight line. A line is also plotted showing
...
single_point_results <- dat %>%
  group_by(condition) %>%
  summarise(single_point_b_basis = single_point_fcn(strength, batch, condition, b_basis_fcn, 0.90),
            single_point_a_basis = single_point_fcn(strength, batch, condition, a_basis_fcn, 0.99),
...
Next, we'll calculate the A- and B-Basis values using the pooling method.

```{r}
pooled_b_results <- dat %>%
  basis_pooled_cv(strength, condition, batch,
                  override = c("between_group_variability",
                               "normalized_variance_equal"))
...
The returned object has a property called `basis`, which is a `data.frame`
for the pooling methods.

```{r}
pooled_b_results$basis
```
...
```{r}
b_basis_pooled_results <- (dat %>%
  basis_pooled_cv(strength, condition, batch,
                  override = c("between_group_variability",
                               "normalized_variance_equal")))$basis %>%
  rename(condition = group,
         b_basis_pooled = value) %>%
  mutate(condition = ordered(condition, c("CTD", "RTD", "ETW", "ETW2")))
...
And the same thing for A-Basis:

```{r}
a_basis_pooled_results <- (dat %>%
